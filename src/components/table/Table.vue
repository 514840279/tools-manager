  ...
  <div>
    <transition name="el-zoom-in-top">
      <div :class="show == 'page' ? 'transition-box' : null" v-show="show == 'page'">
        ...
        <el-row class="brow">
          <el-col :span="18">
            <TableSearch v-if="localOptionBtn.search" :searchColumns="searchColumns"
              v-model:searchParameters="searchParameters" @searchTable="initTable"></TableSearch>
            <slot name="headSearch"></slot>
          </el-col>
          <el-col :span="6">
            <div style="text-align: right; margin-right: 10px;">
              <slot name="rightBtn"></slot>
              <el-button title="查询" v-if="localOptionBtn.searchParam" @click="handleShowShearch()" :type="searchBtnType"
                icon="Search" circle size="small"></el-button>
              <el-button title="排序" v-if="localOptionBtn.sort" @click="handleShowSort()" :type="sortBtnType" icon="Sort"
                circle size="small"></el-button>
              <el-button title="重置" @click="resetTable()" icon="Setting" circle size="small"></el-button>
              <el-button title="添加" v-if="localOptionBtn.add" @click="handleAdd()" type="success"
                icon="CirclePlusFilled" circle size="small"></el-button>
        ...
        <TableSearchParameters v-if="localOptionBtn.searchParam && showSearch" :searchColumns="searchColumns"
          v-model:searchParameters="searchParameters" @searchTable="searchTable"></TableSearchParameters>
        <TableSortParameters v-if="localOptionBtn.sort && showSort" :sortColumns="sortColumns"
          v-model:sortParameters="sortParameters" @sortTable="initTable"></TableSortParameters>
        <div id="Table" ref="print">
          <el-table id="tableid" v-loading="loading" :data="dataList" style="width: 100%" default-expand-all>

            <el-table-column v-for="(column, ind) in activeColumns" :key="ind" :prop="column.name" :label="column.title"
              :width="column.width == null ? '' : column.width" :align="column.align == null ? 'left' : column.align"
              :resizable="column.resizable == null ? false : column.resizable" :type="column.type">
              <template #default="scope">
                ...
                <div v-if="column.searchType == SearchType.OPERATION">
                  <el-button @click="onClickRow(scope.$index, scope.row, column.name)" size="small">{{ column.title }}
                  </el-button>
                </div>
              </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="124" v-if="localOptionBtn.opt">
              <template #default="scope">
                <el-button @click="handleUpd(scope.row)" v-if="localOptionBtn.opt && localOptionBtn.optbtn?.upd"
                  type="success" icon="Edit" circle size="small" title="修改"></el-button>
                <el-button @click="handleStateToDown(scope.row)"
                  v-if="localOptionBtn.opt && localOptionBtn.optbtn?.state && scope.row.deleteFlag == 0 || scope.row.deleteFlag == null"
                  type="warning" icon="View" circle size="small" title="已启用"></el-button>
                <el-button @click="handleStateToUp(scope.row)"
                  v-if="localOptionBtn.opt && localOptionBtn.optbtn?.state && scope.row.deleteFlag == 1" type="info"
                  icon="View" circle size="small" title="已禁用"></el-button>
                <el-button @click="handleDelete(scope.row)" v-if="localOptionBtn.opt && localOptionBtn.optbtn?.del"
                  type="danger" icon="Delete" circle size="small" title="删除"></el-button>
              </template>
            </el-table-column>
          </el-table>
          <div v-if="localOptionBtn.page" class="apagination">
            <el-row>
              <el-col :span="12" :offset="12">
                <el-pagination class="pagex" background @size-change="handleSizeChange"
                  @current-change="handleCurrentChange" :current-page="param.pageNumber" :page-sizes="page.sizes"
                  :page-size="param.pageSize" :pager-count="5" layout="total, sizes, prev, pager, next, jumper"
    ...
    <transition name="el-zoom-in-top">
      <div :class="show == 'edit' ? 'transition-box' : null" v-if="show == 'edit'">
        <TableSave v-if="show == 'edit'" :columns="columns" v-model:info="currentRow" @onSave="handleSave"
          @onCancel="initTable"> </TableSave>
      </div>
    </transition>
  </div>
</template>
...
const emit = defineEmits(["onClickRow", "multipleSelected"]);
